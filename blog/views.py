...
def index(request):
    prefetch_posts = Post.objects.prefetch_related('author')
    most_popular_posts = prefetch_posts.annotate(likes_count=Count('likes')).order_by('-likes_count')
    ...
    top_popular_posts = list(most_popular_posts)[:5]
    ...
    tags = Tag.objects.annotate(num_tags=Count('posts', distinct=True))
    popular_tags = tags.order_by('-num_tags')
    most_popular_tags = list(popular_tags)[:5]

    context = {
        'most_popular_posts': [
            serialize_post_optimized(post) for post in top_popular_posts
        ],
